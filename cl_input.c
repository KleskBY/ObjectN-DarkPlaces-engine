...
kbutton_t	in_strafe, in_jump, in_attack, in_use;
...
static void CL_AdjustAngles (void)
{
	...
	if (in_button4.state & 1) speed = cl.realframetime * cl_anglespeedkey.value;
	else speed = cl.realframetime;
...
void CL_Input (void)
{
	...
	cl.cmd.upmove += cl_upspeed.value * CL_KeyState (&in_jump);
	cl.cmd.upmove -= cl_upspeed.value * CL_KeyState (&in_button3);
	...
	if (in_button4.state & 1)	//walk
	{
		cl.cmd.forwardmove *= 0.5;
		cl.cmd.sidemove *= 0.5;
		cl.cmd.upmove *= 0.5;
	}
	else if (in_button3.state & 1)	//crouch
	{
		cl.cmd.forwardmove *= 0.4;
		cl.cmd.sidemove *= 0.4;
		cl.cmd.upmove *= 0.4;
	}
	...
	if (cl.csqc_loaded && cl_csqc_generatemousemoveevents.integer)
	{
		...
		else if (in_mouse_x || in_mouse_y) CL_VM_InputEvent(2, in_mouse_x, in_mouse_y);
	}
...
static void CL_ClientMovement_Physics_Swim(cl_clientmovement_state_t *s)
{
	...
	if (wishspeed) VectorScale(wishvel, 1 / wishspeed, wishdir);
	else VectorSet( wishdir, 0.0, 0.0, 0.0 );
	wishspeed = min(wishspeed, cl.movevars_maxspeed) * 0.7;

	if (s->crouched) wishspeed *= 0.5;
...
void CL_UpdateMoveVars(void)
{
	...
	if(cl.movevars_aircontrol_power <= 0) cl.movevars_aircontrol_power = 2; // CPMA default
}
...
void CL_InitInput (void)
{
	...
	// LordHavoc: added 6 new buttons
	Cmd_AddCommand ("+duck", IN_Button3Down, "Crouch");
	Cmd_AddCommand ("-duck", IN_Button3Up, "Stop crouching");
	Cmd_AddCommand ("+speed", IN_Button4Down, "Walk");
	Cmd_AddCommand ("-speed", IN_Button4Up, "Stop walking");
