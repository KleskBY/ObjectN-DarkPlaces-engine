...
//	Cvar_RegisterVariable(&r_subdivide_size);
	...
	//if(gamemode == GAME_NEXUIZ) //KleskBY
	...
	for (i = 0;i < nummiptex;i++)
	{
		...
		if (cls.state != ca_dedicated)
		{
			...
			if (!loadmodel->brush.ishlbsp && !strncmp(tx->name, "sky", 3) && mtwidth == mtheight * 2)
			{
				...
				if (!data) data = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "textures/%s", tx->name), false, false, false, NULL);
			...
			else
			{
				...
				if (!skinframe) skinframe = R_SkinFrame_LoadExternal(va(vabuf, sizeof(vabuf), "textures/%s", tx->name), TEXF_ALPHA | TEXF_MIPMAP | TEXF_ISWORLD | TEXF_PICMIP | TEXF_COMPRESS, false, false);
				if (skinframe)
