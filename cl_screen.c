...
cvar_t scr_conalpha2factor = {CVAR_SAVE, "sðcr_conalpha2factor", "0", "opacity of console background gfx/conback2 relative to scr_conalpha; when 0, gfx/conback2 is not drawn"};
...
void SCR_UpdateVars(void)
{
	...
	if (cl.intermission) sb_lines = 0;
	else
	{
		if (scr_viewsize.value >= 120) sb_lines = 0;		// no status bar at all
		else  sb_lines = 24;		// no inventory
	}
}
