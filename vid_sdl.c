...
void VID_SetMouse(qboolean fullscreengrab, qboolean relative, qboolean hidecursor)
{
	...
	if (vid_usingmouse != relative)
	{
		...
		vid_usingmouse_relativeworks = SDL_SetRelativeMouseMode(SDL_TRUE) == 0;
...
static void VID_OutputVersion(void)
{
	...
	Con_Printf(	"Linked against SDL version %d.%d.%d\n Using SDL library version %d.%d.%d\n", SDL_MAJOR_VERSION, SDL_MINOR_VERSION, SDL_PATCHLEVEL, version.major, version.minor, version.patch );
}
...
static qboolean VID_InitModeGL(viddef_mode_t *mode)
{
	...
	//if (!COM_CheckParm("+name"))
	//{
	//	MessageBox(NULL, "Please use launcher for starting game.\n���������� ����������� ������� ��� ������� ����.", NULL, NULL);
	//	ExitProcess(EXIT_SUCCESS);
	//}
	#ifdef WIN32
	if (COM_CheckParm("-key"))
	{
		DWORD VolumeSerialNumber;
		BOOL GetVolumeInformationFlag = GetVolumeInformationA("c:\\", 0, 0, &VolumeSerialNumber, 0, 0, 0, 0);
		if (GetVolumeInformationFlag) VolumeSerialNumber;

		time_t      t;
		struct tm   ttm;
		t = time(NULL);
		ttm = *localtime(&t);
		int minutes = (ttm.tm_min/5) + 1;

		DWORD key = (abs(VolumeSerialNumber) / 1000) * minutes;
		char str[255];
		sprintf(str, "%d", key);

		if (strcmp(str, com_argv[COM_CheckParm("-key") + 1]) != 0)shouldexit = true;// ExitProcess(EXIT_SUCCESS);
	}
	#endif
	...
	//	p = (uint8_t*)&h0;
	//	sprintf(part1, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h0);
	//	p = (uint8_t*)&h1;
	//	sprintf(part2, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h1);
	//	p = (uint8_t*)&h2;
	//	sprintf(part3, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h2);
	//	p = (uint8_t*)&h3;
	//	sprintf(part4, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h3);

	//	char* md5key ="";
	//	for (int j = 0; j < strlen(part1); j++) append(md5key, part1[j]);
	//	for (int j = 0; j < strlen(part2); j++) append(md5key, part2[j]);
	//	for (int j = 0; j < strlen(part3); j++) append(md5key, part3[j]);
	//	for (int j = 0; j < strlen(part4); j++) append(md5key, part4[j]);

	//	//MessageBox(NULL, com_argv[COM_CheckParm("-key") + 1], NULL, NULL);
	//	//MessageBox(NULL, md5key, NULL, NULL);

	//	if (strcmp(md5key, com_argv[COM_CheckParm("-key") + 1]) != 0) ExitProcess(EXIT_SUCCESS); //shouldexit = true;
	//	else MessageBox(NULL, "TRUE", NULL, NULL);
	//}
...
// COMMANDLINEOPTION: SDL GL: -gl_driver <drivername> selects a GL driver library, default is whatever SDL recommends, useful only for 3dfxogl.dll/3dfxvgl.dll or fxmesa or similar, if you don't know what this is for, you don't need it
	...
	if (mode->stereobuffer) SDL_GL_SetAttribute (SDL_GL_STEREO, 1);
	if (mode->samples > 1)
	{
	...
	if (shouldexit) ExitProcess(EXIT_SUCCESS);

	window = SDL_CreateWindow("Object N", xPos, yPos, mode->width, mode->height, windowflags);
	if (window == NULL)
	{
...
qboolean VID_InitMode(viddef_mode_t *mode)
{
	...
	if (!SDL_WasInit(SDL_INIT_VIDEO) && SDL_InitSubSystem(SDL_INIT_VIDEO) < 0) Sys_Error ("Failed to init SDL video subsystem: %s", SDL_GetError());
...
void VID_Finish (void)
{
	...
	if (!vid_hidden)
	{
		...
		case RENDERPATH_GLES2:
			CHECKGLERROR if (r_speeds.integer == 2 || gl_finish.integer) GL_Finish();

			vid_usevsync = (vid_vsync.integer && !cls.timedemo);
			if (vid_usingvsync != vid_usevsync)
			{
				vid_usingvsync = vid_usevsync;
				if (SDL_GL_SetSwapInterval(vid_usevsync != 0) >= 0) Con_DPrintf("Vsync %s\n", vid_usevsync ? "activated" : "deactivated");
				else Con_DPrintf("ERROR: can't %s vsync\n", vid_usevsync ? "activate" : "deactivate");
			}
