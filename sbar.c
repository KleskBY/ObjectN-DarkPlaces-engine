...
static void sbar_start(void)
{
...

	{
		...
		sb_ammo[0] = Draw_CachePic_Flags("gfx/sb_rocket", CACHEPICFLAG_QUIET);
		sb_ammo[1] = Draw_CachePic_Flags("gfx/sb_shells", CACHEPICFLAG_QUIET); 
		sb_ammo[2] = Draw_CachePic_Flags("gfx/sb_9x18", CACHEPICFLAG_QUIET);
		sb_ammo[3] = Draw_CachePic_Flags("gfx/sb_545x39", CACHEPICFLAG_QUIET);
		sb_ammo[4] = Draw_CachePic_Flags ("gfx/sb_762x54", CACHEPICFLAG_QUIET);
		...
		sb_faces[4][1] = Draw_CachePic_Flags ("gfx/face2", CACHEPICFLAG_QUIET);
...
static void Sbar_DrawInventory (void)
{
	...
	for (i=0 ; i<6 ; i++)
		if (cl.stats[STAT_ITEMS] & (1<<(17+i)))
		{
			...
			if (!(gamemode == GAME_HIPNOTIC || gamemode == GAME_QUOTH) || (i>1)) Sbar_DrawPic (192 + i*16, -16, sb_items[i]);
		}
...
static void Sbar_DrawFace (void)
{
	...
	if(cl.stats[STAT_HEALTH] > 25) Sbar_DrawPic(10, 0, sb_faces[4][0]);
	else Sbar_DrawPic(10, 0, sb_faces[4][1]);
...
void Sbar_ShowFPS(void)
{
	...
	if (fps_strings)
	{
		...
		if (fpsstring[0])
		{
			...
			fps_x = -2;//vid_conwidth.integer - DrawQ_TextWidth(fpsstring, 0, fps_scalex, fps_scaley, true, FONT_INFOBAR);
			if (showfps_framerate < 1000) fps_x = -12;
			DrawQ_Fill(fps_x, fps_y, DrawQ_TextWidth(fpsstring, 0, fps_scalex, fps_scaley, true, FONT_INFOBAR), fps_scaley, 0, 0, 0, 0.5, 0);
...
void Sbar_Draw (void)
{
	...
	{
		if (sb_showscores) Sbar_DrawScoreboard ();
		else if (cl.intermission == 1) Sbar_IntermissionOverlay();
		else if (cl.intermission == 2) Sbar_FinaleOverlay();
		//else // Quake and others
		{
			sbar_x = 0;//(vid_conwidth.integer - 320)/2;
			sbar_y = vid_conheight.integer - SBAR_HEIGHT -8;
			// LordHavoc: changed to draw the deathmatch overlays in any multiplayer mode
			...
			//if (sb_showscores || (cl.stats[STAT_HEALTH] <= 0 && cl_deathscoreboard.integer)) Sbar_DrawScoreboard ();
			/*else*/ if (sb_lines && cl.stats[STAT_HEALTH] > 0)
			{
				...
				for (int i = 0; i < 6; i++)
				{
					if (cl.stats[STAT_ITEMS] & (1 << (17 + i)))
					{
						if (!(i > 1)) Sbar_DrawPic( (vid_conwidth.integer / 2) + i * 16, 16, sb_items[i]);
					}
				}

				if (cl.stats[STAT_HEALTH] < 665)
				{
					Sbar_DrawFace(); // face
					Sbar_DrawNum(32, 0, cl.stats[STAT_HEALTH], 3, cl.stats[STAT_HEALTH] <= 25); // health

					// ammo icon
					{
						int ammoicon_x = vid_conwidth.integer - 32;
						if (cl.stats[STAT_ITEMS] & IT_ROCKETS) Sbar_DrawPic(ammoicon_x, 0, sb_ammo[0]);
						else if (cl.stats[STAT_ITEMS] & IT_SHELLS) Sbar_DrawPic(ammoicon_x, 0, sb_ammo[1]);
						else if (cl.stats[STAT_ITEMS] & IT_9x18) Sbar_DrawPic(ammoicon_x, 0, sb_ammo[2]);
						else if (cl.stats[STAT_ITEMS] & IT_545x39) Sbar_DrawPic(ammoicon_x, 0, sb_ammo[3]);
						else if (cl.stats[STAT_ITEMS] & IT_762x54) Sbar_DrawPic(ammoicon_x, 0, sb_ammo[4]);
					}

					Sbar_DrawNum(vid_conwidth.integer - 108, 0, cl.stats[STAT_AMMO], 3, cl.stats[STAT_AMMO] <= 10);
				}

				// LordHavoc: changed to draw the deathmatch overlays in any multiplayer mode
				if ((!cl.islocalgame || cl.gametype != GAME_COOP))
				{
					//Sbar_MiniDeathmatchOverlay (sbar_x + 324, vid_conheight.integer - 8*8); // Хуйня таблист справа внизу экрана
					Sbar_Score(24);
				}
			}
		}
...
void Sbar_DeathmatchOverlay (void)
{
	...
	DrawQ_Fill(xmin, ymin+ Draw_GetPicHeight(sb_ranking), xmax- xmin, ymax- ymin, 0, 0, 0, 0.5, 0);
	// draw the text
	y = 32;
	if (cls.protocol == PROTOCOL_QUAKEWORLD)
	{
	...
	{
		DrawQ_Fill(xmin, y, xmax -xmin, 8, 1.f, 1.f, 1.f, 0.5, 0);
		DrawQ_String(xmin, y, va(vabuf, sizeof(vabuf), "PING PL%% FRAGS  NAME"), 0, 8, 8, 1, 1, 1, 1 * sbar_alpha_fg.value, 0, NULL, false, FONT_SBAR );
	}
	y += 8;

	if (Sbar_IsTeammatch ())	//team scoreborad
	{
		// show team scores first

		for (i = 0; i < teamlines && y < vid_conheight.integer; i++)
		{
			//sprintf(pcol, "%3i//%3i", i, teamlines);
			//DrawQ_String(-2, y, pcol, 0, 8, 8, 1, 1, 1, 1 * sbar_alpha_fg.value, 0, NULL, false, FONT_SBAR);
			if(((teams + teamsort[i])->colors & 15) == 0) DrawQ_Fill(xmin, y, xmax - xmin, 8, 1.f, 0.f, 0.f, 0.5, 0);
			else if (((teams + teamsort[i])->colors & 15) == 1) DrawQ_Fill(xmin, y, xmax - xmin, 8, 0.f, 0.f, 1.f, 0.5, 0);
			else if (((teams + teamsort[i])->colors & 15) == 15) DrawQ_Fill(xmin, y, xmax - xmin, 8, 1.f, 1.f, 1.f, 0.5, 0);
			y += (int)Sbar_PrintScoreboardItem((teams + teamsort[i]), xmin, y);
			for (int j = 0; j < scoreboardlines && y < vid_conheight.integer; j++)
			{
				//sprintf(pcol, "%3i//%3i", ((cl.scores + fragsort[j])->colors & 15)), ((cl.scores + fragsort[j])->colors);
				//DrawQ_String(-2, y, pcol, 0, 8, 8, 1, 1, 1, 1 * sbar_alpha_fg.value, 0, NULL, false, FONT_SBAR);
				//if (((cl.scores + fragsort[j])->colors & 15) == 1) DrawQ_Fill(xmin, y, xmax - xmin, 8, 1.f, 0.f, 0.f, 0.5, 0);
				//else if (((cl.scores + fragsort[j])->colors & 15) == 0)  DrawQ_Fill(xmin, y, xmax - xmin, 8, 0.f, 0.f, 1.f, 0.5, 0);
				if (((cl.scores + fragsort[j])->colors & 15) == ((teams + teamsort[i])->colors & 15)) y += (int)Sbar_PrintScoreboardItem(cl.scores + fragsort[j], xmin, y);
			}
		}
		y += 5;
	}
...
// LA: Display as "a" instead of "a/b" if b is 0
	if(cl.stats[STAT_TOTALSECRETS])
	{
		...
		DrawQ_String(sbar_x + 250, sbar_y + 104, "/", 0, 24, 24, 1, 1, 1, sbar_alpha_fg.value, 0, NULL, false, FONT_SBAR);
	...
	if(cl.stats[STAT_TOTALMONSTERS])
	{
		...
		DrawQ_String(sbar_x + 240, sbar_y + 144, "/", 0, 24, 24, 1, 1, 1, sbar_alpha_fg.value, 0, NULL, false, FONT_SBAR);
