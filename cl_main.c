...
static void CL_UpdateNetworkEntity(entity_t *e, int recursionlimit, qboolean interpolate)
{
	...
	if (cl_nolerp.integer || cls.timedemo) interpolate = false;
...
static void CL_UpdateViewModel(void)
{
	...
	if ((cl.stats[STAT_HEALTH] <= 0 && cl_deathnoviewmodel.integer) || cl.intermission) ent->state_current.modelindex = 0;
	else if (cl.stats[STAT_ITEMS] & IT_INVISIBILITY) ent->state_current.modelindex = 0;
