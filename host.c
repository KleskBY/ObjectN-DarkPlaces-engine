...
// COMMANDLINEOPTION: Server: -listen [playerlimit] starts a multiplayer server with graphical client, like singleplayer but other players can connect, default playerlimit is 8
	...
	if (i || !cl_available)
	{
		...
		if (i && i + 1 < com_argc && atoi (com_argv[i+1]) >= 1) svs.maxclients = atoi (com_argv[i+1]);
		if (COM_CheckParm ("-listen")) Con_Printf ("Only one of -dedicated or -listen can be specified\n");
	...
	else if (cl_available)
	{
		...
		if (i)
		{
			...
			if (i + 1 < com_argc && atoi (com_argv[i+1]) >= 1) svs.maxclients = atoi (com_argv[i+1]);
		}
		else svs.maxclients = 1;
	}
	...
	if (svs.maxclients > 1 && !deathmatch.integer && !coop.integer) Cvar_SetValueQuick(&deathmatch, 1);
}
...
void Host_Main(void)
{
	...
	for (;;)
	{
		...
		if (slowmo.value < 0.00001 && slowmo.value != 0) Cvar_SetValue("slowmo", 0);
		if (host_framerate.value < 0.00001 && host_framerate.value != 0) Cvar_SetValue("host_framerate", 0);
...
//			R_TimeReport("sleep");
		...
		if (cl_timer > 0.1) cl_timer = 0.1;
		if (sv_timer > 0.1)
		{
			if (!svs.threaded) svs.perf_acc_lost += (sv_timer - 0.1);
			sv_timer = 0.1;
		}
